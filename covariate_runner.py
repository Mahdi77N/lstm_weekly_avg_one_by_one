...
def model_on_val(data, h, end_date, model_type, dropout_value, lr_rate):
	...
	for c in range(h, limit+1, h):	# step size is for going through covariates
...
# 	send_mail(mail_subject, mail_body, "", zipf_name)
	...
	with mp.Pool(mp.cpu_count()) as pool:
		model_results = pool.starmap(model_on_val_fixed_param, [(data_tmp, h_tmp, c_tmp, end_date_var, model_type, 0.2, 0.000009) 
		for model_type in range(len(types))])

	tmp = list(itertools.chain.from_iterable(model_results))
	model_results_df = pd.DataFrame(tmp)

	model_results_df.to_csv('Results/model_type_search_models_results_'+str(point)+'.csv', index=False)

	tmp_df = pd.read_csv('Results/model_type_search_models_results_'+str(point)+'.csv')

	best_model_type, _, _, _ = get_best_model_parameters(tmp_df)
	best_model_type = int(best_model_type)
	###########################################################

	########################################################### choosing best lr_rate
	best_lr_rate = -1.0
	...
	with mp.Pool(mp.cpu_count()) as pool:
		model_results = pool.starmap(model_on_val_fixed_param, [(data_tmp, h_tmp, c_tmp, end_date_var, model_type_tmp, 0.2, lr_rate) 
		for lr_rate in lr_list])

	tmp = list(itertools.chain.from_iterable(model_results))
	model_results_df = pd.DataFrame(tmp)

	model_results_df.to_csv('Results/lr_search_models_results_'+str(point)+'.csv', index=False)

	tmp_df = pd.read_csv('Results/lr_search_models_results_'+str(point)+'.csv')

	_, _, best_lr_rate, _ = get_best_model_parameters(tmp_df)
	###########################################################

	########################################################### choosing best dropout value
	...
	with mp.Pool(mp.cpu_count()) as pool:
		model_results = pool.starmap(model_on_val_fixed_param, [(data_tmp, h_tmp, c_tmp, end_date_var, model_type_tmp, dropout, lr_rate_tmp) 
		for dropout in dropout_list])

	tmp = list(itertools.chain.from_iterable(model_results))
	model_results_df = pd.DataFrame(tmp)

	model_results_df.to_csv('Results/dropout_search_models_results_'+str(point)+'.csv', index=False)

	tmp_df = pd.read_csv('Results/dropout_search_models_results_'+str(point)+'.csv')

	_, best_dropout_value, _, _ = get_best_model_parameters(tmp_df)
	###########################################################
